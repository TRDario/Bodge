...
static std::string format_replay_text(std::optional<replay_map::const_iterator> it)
{
	if (!it.has_value()) {
		return std::string{"-------------------------------------------------"};
	}
...
replay_widget::replay_widget(tweened_position pos, tr::align alignment, ticks unhide_time, replays_state& state,
	...
	, text_button_widget{
		  ...
		  font::LANGUAGE,
		  34,
		  [this] { return m_state.m_substate == replays_state::substate::IN_REPLAYS && m_it.has_value(); },
		  [this] {
			  if (m_it.has_value()) {
				  m_state.m_substate = replays_state::substate::STARTING_REPLAY;
				  m_state.m_elapsed = 0;
				  m_state.set_up_exit_animation();
				  g_audio.fade_song_out(0.5s);
				  m_state.m_next_state = make_game_state_async<replay_game>(game_type::REPLAY, fade_in::YES, replay{(*m_it)->first});
			  }
		  },
		  sound::CONFIRM}
{
