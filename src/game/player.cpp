...
player::player(const player_settings& settings, ticks prev_pb)
	: m_hitbox{{500, 500}, settings.hitbox_radius}
	, m_trail{m_hitbox.c}
	, m_lives_left{int(settings.starting_lives)}
...
bool player::colliding_with_any_of(const tr::static_vector<ball, 255>& balls)
{
	return m_iframes_left == 0 &&
		   std::ranges::any_of(balls, [&](const ball& b) { return b.tangible() && tr::intersecting(b.hitbox(), m_hitbox); });
}
...
void player::add_trail_to_renderer(tr::rgb8 tint, std::uint8_t opacity, tr::angle rotation, float size) const
{
	...
	for (std::size_t i = 0; i < m_trail.SIZE; ++i) {
		...
		const std::uint8_t trail_opacity{std::uint8_t(opacity / 3.0f * trail_fade)};
		...
		for (int j = 0; j < 6; ++j) {
			...
			*indices_it++ = std::uint16_t(trail_mesh.base_index + 6 * (i + 1) + j);
			*indices_it++ = std::uint16_t(trail_mesh.base_index + 6 * (i + 1) + ((j + 1) % 6));
			*indices_it++ = std::uint16_t(trail_mesh.base_index + 6 * i + ((j + 1) % 6));
			*indices_it++ = std::uint16_t(trail_mesh.base_index + 6 * (i + 1) + j);
			*indices_it++ = std::uint16_t(trail_mesh.base_index + 6 * i + ((j + 1) % 6));
			*indices_it++ = std::uint16_t(trail_mesh.base_index + 6 * i + j);
		}
	}
}
...
void player::add_timer_to_renderer() const
{
	...
	if (game_over()) {
		tint = (m_timer > m_prev_pb) ? "00FF00"_rgba8 : "FF0000"_rgba8;
		scale = 1;
	}
	else {
...
{
	const float t{(m_game_over_timer + 1) / float(0.5_s)};
