...
tr::next_state game_state::handle_event(const tr::sys::event& event)
{
	return event | tr::match{
					   [this](tr::sys::key_down_event event) -> tr::next_state {
						   if (to_base(m_substate) != substate_base::FADING_IN && event.key == "Escape"_k) {
							   g_audio.play_sound(sound::PAUSE, 0.8f, 0.0f);
							   g_audio.pause_song();
							   return std::make_unique<pause_state>(m_game, to_type(m_substate), g_mouse_pos, true);
						   }
						   return tr::KEEP_STATE;
					   },
					   [](auto) -> tr::next_state { return tr::KEEP_STATE; },
				   };
}
