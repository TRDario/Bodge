...
pause_state::pause_state(active_game&& game, bool blur_in) noexcept
	: _substate{blur_in ? substate::PAUSING : substate::PAUSED}, _timer{0}, _game{std::move(game)}
{
	...
	basic_text_widget& title{_ui.emplace<basic_text_widget>("paused", vec2{500, -50}, CENTER, font::LANGUAGE, 64)};
	...
	for (int i = 0; i < BUTTONS.size(); ++i) {
		...
		clickable_text_widget& widget{
			_ui.emplace<clickable_text_widget>(BUTTONS[i], pos, CENTER, 48, status_cb, std::move(action_cbs[i]), std::move(chords[i]))};
...
unique_ptr<state> pause_state::update(tr::duration)
{
	...
	case substate::RESTARTING:
		return _timer >= 0.5_s ? make_unique<game_state>(active_game{_game.gamemode()}, true) : nullptr;
