...
{
	widget& title{_ui.emplace<text_widget>("enter_your_name", vec2{500, -50}, TOP_CENTER, font::LANGUAGE, ttf_style::NORMAL, 64)};
	...
	line_input_widget<20>& input{_ui.emplace<line_input_widget<20>>("input", vec2{500, 500}, CENTER, 64, input_status_cb, action_cb)};
	input.unhide(1.0_s);
	...
	widget& confirm{_ui.emplace<clickable_text_widget>("confirm", vec2{500, 1050}, BOTTOM_CENTER, font::LANGUAGE, 48, DEFAULT_TEXT_CALLBACK,
													   confirm_status_cb, action_cb)};
...
unique_ptr<state> name_entry_state::update(tr::duration)
{
	...
	case substate::ENTERING_TITLE:
		return _timer >= 1.0_s ? make_unique<title_state>(std::move(_game)) : nullptr;
	}
}
