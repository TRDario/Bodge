	...
	: _substate{substate::IN_EDITOR}, _timer{0}, _game{std::move(game)}, _gamemode{gamemode}
{
	...
	const status_callback status_cb{
		[this] { return _substate == substate::IN_EDITOR; },
	};
	...
	const status_callback max_count_inc_status_cb{
		[this] { return _substate == substate::IN_EDITOR && _gamemode.ball.max_count < 255; },
	};
	...
	const status_callback size_step_dec_status_cb{
		[this] { return _substate == substate::IN_EDITOR && _gamemode.ball.size_step > 0; },
	};
	const status_callback size_step_inc_status_cb{
		[this] { return _substate == substate::IN_EDITOR && _gamemode.ball.size_step < 50.0f; },
	};
	...
	const action_callback size_step_dec_action_cb{
		[&size_step = _gamemode.ball.size_step] { size_step = std::max(size_step - engine::keymods_choose(0.1f, 1.0f, 10.0f), 0.0f); }};
	const action_callback size_step_inc_action_cb{
		[&size_step = _gamemode.ball.size_step] { size_step = std::min(size_step + engine::keymods_choose(0.1f, 1.0f, 10.0f), 50.0f); }};
	...
	const text_callback cur_spawn_interval_text_cb{[this](auto&) { return std::format("{:.1f}s", _gamemode.ball.spawn_interval / 1_sf); }};
	const text_callback cur_initial_size_text_cb{[this](auto&) { return std::format("{:.0f}", _gamemode.ball.initial_size); }};
	const text_callback cur_size_step_text_cb{[this](auto&) { return std::format("{:.1f}", _gamemode.ball.size_step); }};
