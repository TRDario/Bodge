...
ball_settings_editor_state::ball_settings_editor_state(std::shared_ptr<subsystems> subsystems, std::shared_ptr<playerless_game> game,
	...
	, m_pending{std::move(gamemode)}
{
	...
	m_ui.emplace<arrow_widget>(T_STARTING_COUNT_D, {
		...
		.action = [this] {
			m_pending.ball.starting_count = u8(std::max(m_pending.ball.starting_count - m_subsystems->input.choose(1, 5, 10), 1));
		}
	});
	...
	m_ui.emplace<arrow_widget>(T_STARTING_COUNT_I, {
		...
		.action = [this] {
			m_pending.ball.starting_count = u8(std::min(m_pending.ball.starting_count + m_subsystems->input.choose(1, 5, 10), int(m_pending.ball.max_count)));
		}
	});
	m_ui.emplace<arrow_widget>(T_MAX_COUNT_D, {
		...
		.action = [this] {
			m_pending.ball.max_count = u8(std::max({1, int(m_pending.ball.starting_count), m_pending.ball.max_count - m_subsystems->input.choose(1, 5, 10)}));
		}
	});
	...
	m_ui.emplace<arrow_widget>(T_MAX_COUNT_I, {
		...
		.action = [this] {
			m_pending.ball.max_count = u8(std::min(m_pending.ball.max_count + m_subsystems->input.choose(1, 5, 10), 255));
		}
	});
	m_ui.emplace<arrow_widget>(T_SPAWN_INTERVAL_D, {
		...
		.action = [this] {
			m_pending.ball.spawn_interval = ticks(std::max(int(m_pending.ball.spawn_interval - m_subsystems->input.choose(0.1_s, 1_s, 10_s)), int(1_s)));
		}
	});
	...
	m_ui.emplace<arrow_widget>(T_SPAWN_INTERVAL_I, {
		...
		.action = [this] {
			m_pending.ball.spawn_interval = std::min(m_pending.ball.spawn_interval + m_subsystems->input.choose(0.1_s, 1_s, 10_s), 60_s);
		}
	});
	m_ui.emplace<arrow_widget>(T_INITIAL_SIZE_D, {
		...
		.action = [this] {
			m_pending.ball.initial_size = std::max(m_pending.ball.initial_size - m_subsystems->input.choose(1, 5, 10), 10.0f);
		}
	});
	...
	m_ui.emplace<arrow_widget>(T_INITIAL_SIZE_I, {
		...
		.action = [this] {
			m_pending.ball.initial_size = std::min(m_pending.ball.initial_size + m_subsystems->input.choose(1, 5, 10), 100.0f);
		}
	});
	m_ui.emplace<arrow_widget>(T_SIZE_STEP_D, {
		...
		.action = [this] {
			m_pending.ball.size_step = std::max(m_pending.ball.size_step - m_subsystems->input.choose(0.1f, 1.0f, 2.5f), 0.0f);
		}
	});
	...
	m_ui.emplace<arrow_widget>(T_SIZE_STEP_I, {
		...
		.action = [this] {
			m_pending.ball.size_step = std::min(m_pending.ball.size_step + m_subsystems->input.choose(0.1f, 1.0f, 2.5f), 10.0f);
		}
	});
	m_ui.emplace<arrow_widget>(T_INITIAL_VELOCITY_D, {
		...
		.action = [this] {
			m_pending.ball.initial_velocity = std::max(m_pending.ball.initial_velocity - m_subsystems->input.choose(1, 10, 100), 100.0f);
		}
	});
	...
	m_ui.emplace<arrow_widget>(T_INITIAL_VELOCITY_I, {
		...
		.action = [this] {
			m_pending.ball.initial_velocity = std::min(m_pending.ball.initial_velocity + m_subsystems->input.choose(1, 10, 100), 5000.0f);
		}
	});
	m_ui.emplace<arrow_widget>(T_VELOCITY_STEP_D, {
		...
		.action = [this] {
			m_pending.ball.velocity_step = std::max(m_pending.ball.velocity_step - m_subsystems->input.choose(1, 10, 100), 0.0f);
		}
	});
	...
	m_ui.emplace<arrow_widget>(T_VELOCITY_STEP_I, {
		...
		.action = [this] {
			m_pending.ball.velocity_step = std::min(m_pending.ball.velocity_step + m_subsystems->input.choose(1, 10, 100), 1000.0f);
		}
	});
