...
{
	const tr::bitmap render{g_text_engine.render_text(text, g_text_engine.determine_font(text), text_style::NORMAL, 20, 0, 800)};
	m_last_text = text;
	m_last_size = g_text_engine.text_size(text, g_text_engine.determine_font(text), text_style::NORMAL, 20, 0, 800);
	const glm::vec2 scaled_last_size{m_last_size * g_graphics->render_scale()};
...
void tooltip_manager::add_to_renderer(std::string_view text)
{
	...
	const glm::vec2 tl{glm::min(g_mouse_pos, 1000.0f - m_last_size - 2 * OUTLINE - 2 * PADDING)};

	const tr::gfx::simple_color_mesh_ref outline{g_graphics->basic_renderer.new_color_outline(layer::TOOLTIP, 4)};
	...
	const tr::gfx::simple_color_mesh_ref fill{g_graphics->basic_renderer.new_color_fan(layer::TOOLTIP, 4)};
